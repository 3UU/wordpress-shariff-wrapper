...
if ( isset( $shariff3UU_advanced['fb_id'] ) && isset( $shariff3UU_advanced['fb_secret'] ) ) {
	...
	if ( ( defined( PHP_INT_SIZE ) && PHP_INT_SIZE === '4' ) || ! defined( PHP_INT_SIZE ) ) $fb_app_id = $shariff3UU_advanced['fb_id'];
	else $fb_app_id = absint( $shariff3UU_advanced['fb_id'] );
	...
	$facebookID_json = json_decode( $facebookID, true );
	// store results, if we have some
	if ( isset( $facebookID_json['share']['share_count'] ) ) {
		$share_counts['facebook'] = $facebookID_json['share']['share_count'];
	}
...
// otherwise use the normal way
	...
	// store results, if we have some
	if ( isset( $facebook_json['data']['0']['share_count'] ) ) {
		$share_counts['facebook'] = $facebook_json['data']['0']['share_count'];
	}
